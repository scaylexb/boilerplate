<template>
  <label
    :for="id"
    class="group flex cursor-pointer items-center space-x-3 text-md font-semibold"
  >
    <span class="flex size-6 items-center justify-center">
      ...
      <IconCheckmarkDefault
        class="absolute z-10 size-3 fill-accent opacity-0 peer-checked:opacity-100"
      />
    </span>
    ...
    <slot name="label">
      <span
        v-if="label"
        class="leading-none text-secondary group-checked:text-primary"
      >
        {{ label }}
      </span>
    </slot>
